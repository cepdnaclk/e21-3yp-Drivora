...
int main() {
    ...
    if (!cap.isOpened()) {
        std::cerr << "CRITICAL ERROR: MainHub cannot connect to the camera payload." << std::endl;
        return -1;
    }

    std::cout << "MainHub Camera Online. Initializing LDWS Pipeline..." << std::endl;
    ...
    while (true) {
        ...
        // A 5x5 kernel size is standard for removing asphalt noise while keeping lane edges sharp
